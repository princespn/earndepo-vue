...
import {ADMIN_OTP,REMOVE_FUND,CHECKUSEREXISTED, GET_ADMIN_OTP_STATUS } from '@/adminstore/actions.type';
export default {
  data() {
    return {
      product_ids: {
  ...
  methods: {
    AmountValidation(e) {
      ...
      if (fullname == "") {
        ...
        this.nameErr = "Enetr amount must be at least 10";
      }  else {
        this.nameErr = "";
      }
    },
    ...
    getAdminOtpstatus()
    {
         ...
         .then((resp) => {
          if (resp.code === 200) {
            this.otpstatus = resp.data.remove_fund_otp_status;
            // this.$toast.success(resp.message);
          } else {
            this.$toast.error(resp.message);
          }
    ...
    fund_req() {
      ...
      formData.append('otp', this.otp);
         ...
         this.$store
        .dispatch(REMOVE_FUND, formData)
        .then((response) => {
          if (response.code == 200) {
            this.$toast.success(response.message);
            this.$router.push({ name: "removefundreport" });
          } else {
