...
export default {
  ...
  methods: {
    exportToExcel() {
      ...
      axios.post("user-transfer-balance-report", params).then((resp) => {
        if (resp.data.code === 200) {
          ...
          fileLink.setAttribute("download", "Balancerecieve.xls");
    ...
    productReport() {
      ...
      setTimeout(function () {
        const table = $("#direct-income-report").DataTable({
          ...
          ajax: {
            url: serveApiUserHost + "user-transfer-balance-report",
            type: "POST",
            data: function (d) {
              i = 0;
              i = d.start + 1;
