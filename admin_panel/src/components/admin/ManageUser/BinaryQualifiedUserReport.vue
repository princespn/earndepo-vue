...
// import { ADMIN_QUALIFIED_USER } from "@/adminstore/actions.type";
...
export default {
  ...
  methods: {
    ...
    qualifiedUsersReport() {
      ...
      setTimeout(function () {
        const table = $("#qualified-user-report").DataTable({
          ...
          ajax: {
            url: serveApiUserHost + "getqualifieduser",
            type: "POST",
            data: function (d) {
              i = 0;
              i = d.start + 1;
    ...
    exportToExcel(){
                var params = {frm_date: $('#frm_date').val(), to_date: $('#to_date').val(),user_id: $('#user_id').val(),sponser_id: $('#sponser_id').val(),action:'export',responseType: 'blob' };
                axios.post('getqualifieduser', params).then(resp => {
                    if(resp.data.code === 200){
                        ...
                        fileLink.setAttribute('download', 'Qualified.xls');
