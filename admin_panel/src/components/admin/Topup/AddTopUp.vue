...
import { CHECKUSEREXISTED,ADMIN_OTP,ADMIN_TOPUP,PRODUCT, GET_ADMIN_OTP_STATUS } from "@/adminstore/actions.type";
export default {
  data() {
    return {
      otpErr: "",
  ...
  methods: {
    ...
    getAdminOtpstatus()
    {
         ...
         .then((resp) => {
          if (resp.code === 200) {
            this.otpstatus = resp.data.topup_update_status;
            // this.$toast.success(resp.message);
          } else {
            this.$toast.error(resp.message);
          }
